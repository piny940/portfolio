...
type ProjectTagResolver interface {
	ProjectID(ctx context.Context, obj *domain.ProjectTag) (string, error)
}
...
func (ec *executionContext) _ProjectTag_projectId(ctx context.Context, field graphql.CollectedField, obj *domain.ProjectTag) (ret graphql.Marshaler) {
	...
	resTmp, err := ec.ResolverMiddleware(ctx, func(rctx context.Context) (interface{}, error) {
		ctx = rctx // use context from middleware stack in children
		return ec.resolvers.ProjectTag().ProjectID(rctx, obj)
	})
	if err != nil {
		ec.Error(ctx, err)
		return graphql.Null
	...
	for i, field := range fields {
		...
		case "projectId":
			...
			innerFunc := func(ctx context.Context, fs *graphql.FieldSet) (res graphql.Marshaler) {
				...
				res = ec._ProjectTag_projectId(ctx, field, obj)
				if res == graphql.Null {
					atomic.AddUint32(&fs.Invalids, 1)
				}
				return res
